...
<template>
  <div class="flex flex-col h-full bg-neutral-50 dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800">
    <!-- Header / Filters -->
    <div class="p-4 space-y-4 bg-white dark:bg-neutral-900 shadow-sm z-10 sticky top-0">
      <div>
        <h1 class="text-xl font-bold text-neutral-900 dark:text-white">Bengaluru Churches</h1>
        <p class="text-sm text-neutral-500 dark:text-neutral-400">Discover historic places of worship</p>
      </div>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
      <div v-if="churches.length === 0" class="text-center py-10 text-neutral-500">
        <p>No churches found matching your criteria.</p>
        <UButton 
          variant="link" 
          color="primary" 
          class="mt-2"
          @click="() => { searchQuery = ''; selectedDenomination = 'All'; selectedArea = 'All' }"
        >
          Clear filters
        </UButton>
      </div>
      ...
      >
        <div class="flex justify-between items-start mb-1">
          <h3 class="font-semibold text-neutral-900 dark:text-white">{{ church.name }}</h3>
          <UBadge size="sm" variant="subtle" color="neutral">{{ church.denomination }}</UBadge>
        </div>
        
        <p class="text-sm text-neutral-500 dark:text-neutral-400 mb-2 flex items-center gap-1">
        ...
        <div class="flex flex-wrap gap-1 mt-2">
          ...
          <span v-if="(church.services?.length || 0) > 2" class="text-[10px] text-neutral-400 self-center ml-1">
